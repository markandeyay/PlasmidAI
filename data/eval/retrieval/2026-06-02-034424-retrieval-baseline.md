...
- Generated at: `2026-06-02T03:44:24.622620+00:00`
...
- Top-5 hit rate: `0.950`
- Clarification pass rate: `1.000`
- MRR: `0.900`

## Baseline Comparison

Baseline: `data/eval/retrieval/2026-05-31-221057-retrieval-baseline.md`

| Metric | 2026-05-31 baseline | Expanded corpus rerun | Change |
| --- | ---: | ---: | ---: |
| Top-1 hit rate | `0.700` | `0.850` | `+0.150` |
| Top-5 hit rate | `1.000` | `0.950` | `-0.050` |
| MRR | `0.825` | `0.900` | `+0.075` |
| Clarification pass rate | `1.000` | `1.000` | `0.000` |

Overall rank quality improved, but the Phase 1 top-5 gate regressed on one query and should be reviewed before treating the expanded corpus as the new gate baseline.

## Regression Analysis

Affected query: `Recommend a low-copy bacterial cloning plasmid with chloramphenicol resistance.`

Expected gold target: `pACYC184`.

Observed expanded-corpus result: `pACYC184` moved from rank `3` in the 2026-05-31 baseline to rank `7` in a diagnostic `TOP_K=20` retrieval. The top five are now GenBank chloramphenicol-marked cloning/expression vector records: `genbank:AY428809.1`, `genbank:U80929.2`, `genbank:M37847.1`, `genbank:U72488.1`, and `genbank:AY219685.1`.

Lane diagnosis:

| Lane | Finding |
| --- | --- |
| Lexical name | Not engaged. The query is descriptive and does not name `pACYC184`, so the exact-name lane has no curated-name cue to boost. |
| Structured filters | Passed too broadly rather than failed. The parsed request requires bacterial cloning, `E. coli`, and chloramphenicol evidence; multiple expanded GenBank records and `curated:pACYC184` satisfy those fields. Current structured filters do not encode `low-copy` or p15A-origin preference as hard filters. |
| Semantic ranking | Primary source of the top-5 miss. Expanded GenBank records with strong title/marker text for chloramphenicol vectors rank ahead of `curated:pACYC184`; `pACYC184` remains relevant but scores lower at rank `7`. |

Database self-consistency inspection of the locally stored public-record metadata confirmed the top candidates carry chloramphenicol marker evidence as classified by the system. The inspection was used only to verify retrieval correctness against local rows populated from public NCBI records; no retrieval tuning was attempted in this session.

Recommended review decision: either broaden this gold case to admit newly added valid chloramphenicol vector records when the user asks generally, or add copy-number/origin-aware intent and ranking before keeping `pACYC184` as the only acceptable answer for `low-copy` wording.
...
Retrieved:
- 1. `curated:pGEX-4T-1` pGEX-4T-1 score=`0.6593` fields=`semantic, vector_type, organism, source, tags`
...
- 3. `curated:pGL3-Basic` pGL3-Basic score=`0.6368` fields=`semantic, vector_type, organism`
- 4. `curated:pGL4-10-luc2` pGL4.10[luc2] score=`0.6277` fields=`semantic, vector_type, organism`
...
- 5. `genbank:U47122.2` Cloning vector pSP-luc+, Luciferase cassette vector, complete sequence score=`0.6527` fields=`semantic, vector_type, organism, genes`
...
- 2. `genbank:U80929.2` Cloning vector pBACe3.6, complete sequence score=`0.7539` fields=`semantic, vector_type, organism, markers, application`
- 3. `genbank:M37847.1` Bacterial cloning vector pMMB206, complete genome score=`0.7462` fields=`semantic, vector_type, organism, markers, application`
- 4. `genbank:U72488.1` Cloning vector pRNA8, complete sequence score=`0.7452` fields=`semantic, vector_type, organism, markers, application`
- 5. `genbank:AY219685.1` Expression vector pXC99E, complete sequence score=`0.7245` fields=`semantic, vector_type, organism, markers`
...
- 5. `genbank:AF310245.1` Cloning vector pGEM58ZNf(-), complete sequence score=`0.7660` fields=`semantic, vector_type, organism, markers, application`
...
- 2. `genbank:AF147463.1` T7 Expression vector pNam, complete sequence score=`0.7669` fields=`semantic, vector_type, organism, markers, promoters, application`
- 3. `genbank:U19585.1` Cloning vector pPROEX-1, complete sequence score=`0.7599` fields=`semantic, vector_type, organism, markers`
- 4. `genbank:AF533984.1` Cloning vector pIA423, complete sequence score=`0.7579` fields=`semantic, vector_type, organism, markers, promoters`
- 5. `genbank:AF087042.1` Cloning vector pCALnFLAG, complete sequence score=`0.7455` fields=`semantic, vector_type, organism, markers, promoters`
